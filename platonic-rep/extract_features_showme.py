...
from showme_utils.showo2_single_path_uniencoder_new import ShowO2SinglePathModel_uni
...
def create_showme_model(config_file, ckpt_path):
    ...
    model = ShowO2SinglePathModel_uni(**yaml_conf)
    vision_model = model.showo_model.vision_model
...
def extract_lvm_features(config_file, lvm_model_name, dataset, args):
    ...
    for i in trange(0, len(dataset), args.batch_size):
        with torch.no_grad():
            ...
            image_embeds_siglip = image_embeds_siglip.permute(0, 2, 1)

            # image_embeds = model.showo_model.siglip_proj(image_embeds_siglip)
            image_embeds = image_embeds_siglip
