...
from sit_utils.models import SiT_models
...
def create_showme_model(config_file, ckpt_path):
    ...
    state_dict = find_model(ckpt_path or f"SiT-XL-2-256x256.pt")
...
def extract_lvm_features(config_file, lvm_model_name, dataset, args):
    ...
    vision_model = create_showme_model(config_file=config_file, ckpt_path=None)
    vision_model.cuda()
    args.vae = "mse"
    ...
    for i in trange(0, len(dataset), args.batch_size):
        with torch.no_grad():
            ...
            latent = vae.encode(ims).latent_dist.sample()
            y = torch.tensor(class_labels).repeat(len(latent)).cuda()
            t = torch.tensor([0.0]).repeat(len(latent)).cuda()
            model_out, block_features = vision_model(latent, t, y)
            image_embeds = block_features[19]
